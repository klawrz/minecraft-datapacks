...
execute at @e[tag=amethyst_allay_marker] store result score #allays entities if entity @e[type=allay, distance=..15]

# If there are 2 allays, run the function unless the allays have a specific tag
execute if score #allays entities matches 2 as @e[tag=amethyst_allay] as @s[tag=!has_duplicated] at @e[tag=amethyst_allay_marker] run function klawrz:challenges/crimson_room/amethyst_room/amethyst_allay
...
# Handle memory game
execute as @e[tag=amethyst_memory_marker, tag=!done_memory] run function klawrz:challenges/crimson_room/amethyst_room/amethyst_memory/handle_amethyst_memory
...
# Handle portal
execute as @e[tag=saffron_frame_marker,tag=!tagged_portal] at @s if entity @e[type=glow_item_frame,tag=saffron_frame_01,nbt={ItemRotation:7b,Item:{components:{"minecraft:custom_data":{SaffronKey:1b}}}},dx=0,dy=0,dz=0] if entity @e[type=glow_item_frame,tag=saffron_frame_02,nbt={ItemRotation:1b,Item:{components:{"minecraft:custom_data":{SaffronKey:1b}}}},dx=1,dy=0,dz=0] if entity @e[type=glow_item_frame,tag=saffron_frame_03,nbt={ItemRotation:5b,Item:{components:{"minecraft:custom_data":{SaffronKey:1b}}}},dx=0,dy=0,dz=1] if entity @e[type=glow_item_frame,tag=saffron_frame_04,nbt={ItemRotation:3b,Item:{components:{"minecraft:custom_data":{SaffronKey:1b}}}},dx=1,dy=0,dz=1] run tag @s add tagged_portal
execute as @e[tag=tagged_portal] at @s run function klawrz:challenges/exit_portal
# Portal solution:
# O X
# X X = 1b // Saffron Frame 02
#
# X O
# X X = 3b // Saffrom Frame 04
#
# X X
# O X = 7b // Saffron Frame 01
#
# X X
# X O = 5b // Saffron Frame 03
